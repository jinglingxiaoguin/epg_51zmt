...
def normalize_channel_name(channel_name):
    # 特例处理
    if re.search(r'CCTV.*4K', channel_name, re.IGNORECASE):
        if re.search(r'CCTV[-\s]*16', channel_name, re.IGNORECASE):
            return 'CCTV16'
        else:
            return 'CCTV4K'

    # 一般情况处理
    match = re.match(r'CCTV[-\s]?(\d+)([^\d]*)', channel_name, re.IGNORECASE)
    if match:
        channel_number = match.group(1)
        channel_suffix = match.group(2).strip()
        if channel_suffix in ['+', '体育赛事']:
            return f'CCTV{channel_number}+'
        else:
            return f'CCTV{channel_number}'
...
def single_channel_epg(request):
    ...
    if request.method == "GET" and 'ch' in request.GET and 'date' in request.GET:
        ...
        # 标准化频道名称
        tvg_name = normalize_channel_name(tvg_name)
...
def diyp(request):
    ...
    ret1 = {
        "channel_name": normalize_channel_name(ret['channel']),
